...
# Код проверяем сразу: неполная копия проекта иначе всплыла бы только на
# запуске, после всех вопросов про токен — и человек не понял бы, при чём тут он
PREFLIGHT_OUT="$("$PY" -c 'import bot' 2>&1)"
if [ -n "$PREFLIGHT_OUT" ]; then
    ui_blank
    ui_fail "код проекта неполный или сломан"
    printf '%s\n' "$PREFLIGHT_OUT" | tail -n 6 | ui_trim | sed 's/^/         /'
    die "Установка остановлена" \
        "Скачайте проект заново:" \
        "rm -rf $PROJECT_DIR && повторите команду установки"
fi
ui_ok "код проекта на месте и собирается"
...
install_launcher "$PROJECT_DIR" || true
